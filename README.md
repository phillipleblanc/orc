...
Press **`n`** in the picker to create and attach to a session with the same defaults as `orc new`: the configured agent, the `spiceai-project` project, and an autogenerated verb-noun name. This also works when the list is empty or after switching back with **Ctrl+'**. While filtering, `n` is ordinary search text; use **`/`** to begin a search with `n`, or **Ctrl-U** to clear the filter and restore the new-session shortcut.
